...
class _LearningHubScreenState extends State<LearningHubScreen>
  ...
  Future<void> _loadLearnings() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        final response =
            await _supabaseService.client.from('learnings').select('''
            *,
            lessons:lessons(*)
      ...
      for (final learning in learnings) {
        ...
        if (coverImagePath != null && coverImagePath.isNotEmpty) {
          imageUrl =
              _supabaseService.getPublicUrl('learning-content', coverImagePath);
        ...
        if (user != null) {
          ...
          if (progressData != null && progressData.isNotEmpty) {
            ...
            progress =
                (progressItem['progress_percentage'] as num?)?.toDouble() ??
                    0.0;
  ...
  Widget build(BuildContext context) {
    ...
    final completeCourses =
        filteredCourses.where((c) => c['category'] == 'complete').toList();
    final inProgressCourses =
        filteredCourses.where((c) => c['category'] == 'inprogress').toList();
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => const LearningHubCreateScreen()),
          ).then((newLearning) {
          ...
          if (completeCourses.isNotEmpty) ...[
            _buildSectionHeader('Complete', completeCourses.length > 2, () {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => const LearningHubCompleteScreen()),
              ).then((_) {
                _loadLearnings();
              });
            }, textColor),
            const SizedBox(height: 12.0),
            ...completeCourses.take(2).map((course) =>
                _learningItemCardFromMap(course, cardColor, textColor)),
          ...
          if (inProgressCourses.isNotEmpty) ...[
            _buildSectionHeader('In progress', inProgressCourses.length > 2,
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => const LearningHubInprogressScreen()),
              ).then((_) {
                _loadLearnings();
              });
            }, textColor),
          ...
          if (allCoursesFiltered.isNotEmpty) ...[
            _buildSectionHeader('All learning', allCoursesFiltered.length > 2,
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => const LearningHubAllScreen()),
              ).then((_) {
                _loadLearnings();
              });
            }, textColor),
            const SizedBox(height: 12.0),
            ...allCoursesFiltered.take(2).map((course) =>
                _learningItemCardFromMap(course, cardColor, textColor)),
          ],
          if (isEmpty)
            const Center(
  ...
  Widget _buildSectionHeader(String title, bool showSeeAll,
      VoidCallback onSeeAllPressed, Color textColor) {
    return Row(
  ...
  Widget _learningItemCardFromMap(
      Map<String, dynamic> course, Color cardColor, Color textColor) {
